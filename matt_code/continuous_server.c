...
int process_incoming(char *buffer, int buffer_size, Incoming_Data *data);

void uninitialize_socket(Sockets *socket_data);

int socket_run(Sockets *socket_data, char *buffer, Incoming_Data *data);
...
int main()
{
    ...
    while (1)
    {
        ...
        printf("%s: %d, %d, %d\n", val ? "Something" : "Nothing", incoming_data.state.len, incoming_data.table.len, incoming_data.app.len);
...
// This and the one below are the same
// COMBINE
...
void process_state(char *datagram, int data_len, State_Info *info)
{
    ...
    printf("Test State: %x, %x, %x, %x\n", info->timestamp, info->app_id, info->pointer_offset, info->len);
}
